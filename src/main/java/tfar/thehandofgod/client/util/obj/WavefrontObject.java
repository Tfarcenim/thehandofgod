...
public class WavefrontObject {
    ...
    private static final Pattern face_V_VT_VN_Pattern = Pattern
            .compile("(f( \\d+/\\d+/\\d+){3,} *\\n)|(f( \\d+/\\d+/\\d+){3,} *$)");
    private static final Pattern face_V_VT_Pattern = Pattern.compile("(f( \\d+/\\d+){3,} *\\n)|(f( \\d+/\\d+){3,} *$)");
    private static final Pattern face_V_VN_Pattern = Pattern.compile("(f( \\d+//\\d+){3,} *\\n)|(f( \\d+//\\d+){3,} *$)");
    private static final Pattern face_V_Pattern = Pattern.compile("(f( \\d+){3,} *\\n)|(f( \\d+){3,} *$)");
    private static final Pattern groupObjectPattern = Pattern.compile("([go]( [\\w\\d\\.]+) *\\n)|([go]( [\\w\\d\\.]+) *$)");
    private static final Pattern rotationPattern = Pattern.compile("(rc( [\\d\\-\\.]+){3} *\\n)|(rc( [\\d\\-\\.]+){3} *$)");
        ...
        if (mtl.isEmpty()) {
            ...
            try {
                IResource res = Minecraft.getMinecraft().getResourceManager().getResource(
                        new ResourceLocation(TheHandOfGod.MODID, pathName == null ? line : pathName + line));
                try (BufferedReader reader = new BufferedReader(new InputStreamReader(res.getInputStream()))) {
                    ...
                    while ((currentLine = reader.readLine()) != null) {
                        ...
                        if (currentLine.startsWith("#") || currentLine.length() == 0) {
                            ...
                            if (currentTexture != null) {
                                currentTexture.ka = FloatBuffer
                                        .wrap(new float[]{Float.parseFloat(currentLine.substring(3)) * 128 / 1000});
                            } else {
                                throw new ModelFormatException("File Format Error: " + currentLine);
                            }
                        } else if (currentLine.startsWith("map_Kd ")) {
                            if (currentTexture != null) {
                                currentTexture.texture = new ResourceLocation(TheHandOfGod.MODID,
                                        pathName == null ? currentLine.substring(7)
                                                : pathName + currentLine.substring(7));
